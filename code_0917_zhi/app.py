...
def home():
    ...
    # -----------------------------------------------
#  新增：權限設定頁面 (原本的 ui2.app.py)
# -----------------------------------------------
@app.route('/admin/set_permissions', methods=['GET', 'POST'])
def set_permissions():
    # 這裡可以加上權限檢查，例如：
    ...
    if request.method == 'POST':
        ...
        steps = request.form.getlist("steps") # .getlist() 用來抓取所有勾選的 checkbox

        # 範例：您可能也需要一個帳號欄位來指定是誰的權限
        # account = request.form.get("account") 
        # (注意：HTML 裡的帳號欄位目前是 disabled，您需要先 S_OK)

        # 2. 【後台控制核心】
        #    在這裡，您就可以呼叫 db.py 的功能，把資料存進 domdb.json
        #    (這部分您需要自己或我再協助您根據 domdb.py 的功能來撰寫)
        #
        #    例如 (這只是範例，db.py 中目前沒有這個函式)：
        #    db.update_user_permissions(account, hospital, department, position, steps)
        #
        print(f"後台收到權限資料：院所={hospital}, 科室={department}, 職位={position}, 權限={steps}")

        # 3. 準備 'result' 變數，這樣頁面就會顯示 "✅ 已送出資料"
        result = {
            "hospital": hospital,
    ...
    # 4. 顯示網頁
    # - 如果是 GET 請求 (第一次載入)，result 會是 None
    # - 如果是 POST 請求 (按了確定)，result 會有值
    # 
    # (這裡假設您已經做了【步驟 1】，在 templates 資料夾中建立了 set_permissions.html)
    return render_template('set_permissions.html', result=result)
